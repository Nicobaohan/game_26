...
void Solver::set_camera_model(const cv::Mat & camera_matrix, const cv::Mat & distort_coeffs)
{
  if (camera_matrix.rows != 3 || camera_matrix.cols != 3 || camera_matrix.type() != CV_64F) {
    throw std::invalid_argument("camera_matrix must be a 3x3 CV_64F matrix");
  }
  if (distort_coeffs.empty() || distort_coeffs.type() != CV_64F) {
    throw std::invalid_argument("distort_coeffs must be a non-empty CV_64F matrix");
  }
