...
public:
  ...
  // ROS camera drivers publish the active intrinsic calibration at runtime.
  // Keep the upstream YAML values as an offline fallback, but allow the ROS
  // adapter to replace them with /camera_info from the connected camera.
  void set_camera_model(const cv::Mat & camera_matrix, const cv::Mat & distort_coeffs);
